...
if [[ "${UNITOS_CONTROL_PLANE_PROMOTION:-}" != "PROMOTE_VALIDATED_CONTROL_PLANE_1.4.27_ONLY" ]]; then echo "Bloqueado: autorização específica da promoção ausente" >&2; exit 2; fi
...
SELECT public.promote_control_plane_release(
  ...
  '1.4.27', :'target_commit', :'contract_sha',
...
EXPECTED="1.4.27,1.4.27,$UNITOS_CONTROL_PLANE_TARGET_COMMIT_SHA,$UNITOS_CONTROL_PLANE_CONTRACT_SHA256,$((GENERATION + 1))"
if [[ "$FINAL" != "$EXPECTED" ]]; then echo "Falha: promoção atômica do Control-plane não foi confirmada" >&2; exit 1; fi
echo "Control-plane 1.4.27 promovido atomicamente e validado"