...
FILES=[ROOT/'master'/'operation-state-catalog.json',ROOT/'master'/'002_control_plane_global_freeze.sql',ROOT/'master'/'global-freeze-install-preflight.sql',ROOT/'master'/'003_control_plane_deterministic_update.sql',ROOT/'master'/'deterministic-update-install-preflight.sql',ROOT/'master'/'install-deterministic-update.sql',ROOT/'master'/'004_control_plane_release_promotion.sql',ROOT/'master'/'install-control-plane-release.sql',ROOT/'master'/'005_activate_cron_37.sql']
...
def build():
 d={'schemaVersion':1,'releaseVersion':'1.4.18','operationStateCatalog':'operation-state-catalog.json','files':{p.name:sha(p) for p in FILES},'tables':TABLES,'functions':FUNCTIONS,'triggers':TRIGGERS}
