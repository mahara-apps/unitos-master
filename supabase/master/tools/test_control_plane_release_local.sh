...
if "${PSQL[@]}" -c "SELECT public.promote_control_plane_release(0,'1.3.71','1.3.72','old','1.4.29','new',repeat('a',64),'{\"contractValidated\":true}'::jsonb,'test')" >/dev/null 2>&1; then exit 1; fi
...
"${PSQL[@]}" -c "SELECT public.promote_control_plane_release(0,'1.3.71','1.3.72','old','1.4.29','new',repeat('a',64),'$EVIDENCE'::jsonb,'test')" >/dev/null
"${PSQL[@]}" -Atc "SELECT concat_ws(',',current_version,pinned_release,pinned_commit_sha,generation) FROM public.control_plane_release_state" | grep -qx '1.4.29,1.4.29,new,1'
...
if "${PSQL[@]}" -c "SELECT public.promote_control_plane_release(0,'1.4.29','1.4.29','new','1.4.29','newer',repeat('b',64),'$EVIDENCE'::jsonb,'test')" >/dev/null 2>&1; then exit 1; fi
...
if PGOPTIONS='-c statement_timeout=200' "${PSQL[@]}" -c "SELECT public.promote_control_plane_release(1,'1.4.29','1.4.29','new','1.4.29','newer',repeat('b',64),'$EVIDENCE'::jsonb,'test')" >/dev/null 2>&1; then kill "$holder" 2>/dev/null || true; exit 1; fi
wait "$holder"
