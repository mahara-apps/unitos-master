...
A recuperação histórica é um terceiro modo, bloqueado por `UNITOS_MASTER_RECOVERY=RECOVER_MISSING_1_4_10_ONLY`, `MASTER_PROJECT_REF` idêntico ao manifesto e preflight integralmente `PASS`. O preflight valida a definição completa, corpo, assinatura sem overloads, propriedades, owner, ACL e dependências da 1.4.11. O modo monta uma área temporária contendo exclusivamente `20260919143000_recover_missing_legacy_reconciliation.sql`, exige que o `dry-run` da Supabase CLI selecione somente esse arquivo e bloqueia 1.4.10, 1.4.11 ou qualquer outra migration. A Supabase CLI executa e registra a nova identidade; o SQL não escreve no ledger, nunca repara ou inventa a entrada histórica `20260917184500` e não contém SQL da 1.4.11.
