...
import { buildLegacyReconciliationInspectionSql, legacyEvidenceBlockReason, normalizeLegacyEvidenceRows, reconciledLegacyPositions } from "@/lib/installation/legacy-reconciliation";
...
const rows = [...classifications].map(([position, classification]) => ({ position, migration_file: migrations[position - 1]?.file, classification, evidence_key: `${position}:${migrations[position - 1]?.file}:${classification}`, status: classification === "external_checkpoint_required" ? "insufficient" : "compatible", observed: "teste" }));
...
describe("reconciliação segura do ledger legado", () => {
  ...
  it("mantém compatibilidade parcial distinta de execução histórica", () => {
    ...
    expect(normalized.filter((item) => item.classification === "partial_compatibility")).toHaveLength(7);
    expect(reconciledLegacyPositions(normalized)).toEqual(new Set([34, 39, 55, 61, 64, 70, 71, 84]));
  });
  it("exige checkpoints externos independentes para 72, 74 e 85", () => {
    const normalized = normalizeLegacyEvidenceRows(rows);
    expect(legacyEvidenceBlockReason(normalized)).toContain("checkpoint externo obrigatório 72, 74, 85");
  ...
  it("libera o gate externo sem promover as posições parciais", () => {
    const approved = rows.map((item) => item.classification === "external_checkpoint_required" ? { ...item, status: "compatible" } : item);
    const normalized = normalizeLegacyEvidenceRows(approved);
    expect(legacyEvidenceBlockReason(normalized)).toBeNull();
    expect(reconciledLegacyPositions(normalized)).toEqual(new Set([34, 39, 55, 61, 64, 70, 71, 72, 74, 84, 85]));
    for (const position of [21, 42, 52, 56, 66, 82, 83]) expect(reconciledLegacyPositions(normalized).has(position)).toBe(false);
  });
  ...
  it("é determinística em execução repetida", () => {
    expect(buildLegacyReconciliationInspectionSql(migrations)).toBe(buildLegacyReconciliationInspectionSql(migrations));
