...
export const completeInstallationBootstrap = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (createError || !userId) {
      throw new Error("Não foi possível concluir a ativação. Confira o código e tente novamente.");
    }

    try {
      const { error } = await callRpc(supabaseAdmin, "complete_installation_bootstrap_service", {
        _user_id: userId,
