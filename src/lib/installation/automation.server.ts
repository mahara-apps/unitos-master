...
export async function shouldInterruptOperation(
	...
	const current = await readOperationControlState(
		client,
		operation.id,
		options,
	);
	...
	return (
		operation.fencing_token != null &&
		current.fencing_token !== operation.fencing_token
	);
}
...
function bindAppUrl(sql: string, appUrl: string): string {
	...
	return pure
		.replace(/:'app_url'/g, sqlLiteral(appUrl))
		.replace(/:app_url\b/g, sqlLiteral(appUrl));
}
...
> {
	...
	const runKey = (options?.runKey ?? `legacy:${deltaFingerprint(sql)}`).slice(
		0,
		500,
	);
	...
	if (
		!Number.isInteger(canonicalTotal) ||
		canonicalTotal !== statements.length
	) {
		return {
			ok: false,
	...
	for (let start = from; start < stopAt; start += batchSize) {
		...
		for (const statement of batch) {
			const isEnumAdd = /^\s*alter\s+type\b[\s\S]*\badd\s+value\b/i.test(
				statement,
			);
		...
		for (const segment of segments) {
			...
			if (segment.kind === "enum") {
				for (const statement of segment.statements) {
					...
					if (
						!enumRes.ok &&
						!/already exists|duplicate|does not exist/i.test(
							enumRes.error ?? "",
						)
					) {
					...
					if (!enumRes.ok) {
						const duplicateCheckpoint = await management.query(
							checkpointSql(nextIndex),
						);
			...
			if (!retryDeferred.ok)
				return { ok: false, error: retryDeferred.error, processed };
		}
...
export function extractSupabaseApiKeys(body: unknown): {
	...
	if (body && typeof body === "object") {
		...
		return {
			...
			serviceRoleKey:
				clean(row["service_role_key"]) ?? clean(row["secret_key"]),
		};
	}
	...
	return {
		async readAuth() {
			try {
				...
				const config = (await res.json().catch(() => null)) as Record<
					string,
					unknown
				> | null;
				return config
				...
				return {
					ok: false,
					error:
						error instanceof Error ? error.message : "falha ao ler config/auth",
				};
			}
		},
		async query(sql) {
			...
			for (let attempt = 0; attempt < attempts; attempt++) {
				...
				try {
						...
						try {
							...
							if (!Array.isArray(body)) {
								last = {
									...
									rows: [],
									error:
										"resposta inválida da Management API: esperado um array JSON",
								};
							} else {
					...
					last = {
						...
						rows: [],
						error: aborted
							? "timeout de 12s na Management API"
							: (e as Error).message,
					};
				} finally {
					clearTimeout(timer);
					...
					try {
						...
						if (!res.ok) {
							...
							last = {
								ok: false,
								error:
									"o token leu o projeto, mas não retornou as chaves anon e service_role.",
							};
							break;
						}
					} catch (e) {
...
export async function validateSupabaseProjectKeys(input: {
	...
	const check = async (key: string, label: string) => {
		...
		try {
			const res = await doFetch(
				`${origin}/rest/v1/installation?select=id&limit=1`,
				{
	...
	const publishableError = await check(
...
export function confirmVercelGithubLink(
	...
	const container =
		body && ("link" in body || "gitRepository" in body) ? body : null;
	const link = (container?.gitRepository ??
		container?.link ??
		body ??
		{}) as VercelGitLink;
	const repo = (
...
export function parseDeployQuotaError(
	...
	if (
		status !== 402 &&
		!/api-deployments-free-per-day|payment_required/i.test(text)
	) {
...
export function createCodeClient(input: {
	...
	const fail = async (res: Response, what: string) => {
		...
		if (limited || /api rate limit exceeded|secondary rate limit/i.test(text)) {
			...
			const when = resetAt
				? ` A cota volta em ${formatDateTimeBr(new Date(resetAt * 1000))}.`
				: "";
			return (
		...
		const content =
			body.encoding === "base64" && body.content
				? new TextDecoder().decode(
						Uint8Array.from(atob(body.content.replace(/\s+/g, "")), (c) =>
							c.charCodeAt(0),
						),
					)
				: (body.content ?? "");
		if (!content)
			return { ok: false as const, error: `${path} retornou conteúdo vazio` };
	...
	return {
			...
			try {
				...
				if (!masterRes.ok) {
					return {
						...
						detail: await fail(
							masterRes,
							`ler o repositório do MASTER ${master}`,
						),
					};
				}
				...
				const isPersonal =
					login.toLowerCase() === input.owner.trim().toLowerCase();
		...
		async permissions() {
			...
			try {
				...
				if (quota.ok) {
					...
					push(
						...
						`${remaining}/${core.limit ?? "?"} chamadas restantes${
							core.reset
								? ` — renova em ${formatDateTimeBr(new Date(core.reset * 1000))}`
								: ""
						}`,
					);
				} else {
					push(
						"Cota de uso do GitHub",
						false,
						await fail(quota, "consultar a cota do token"),
					);
				}
				...
				push(
					...
					Boolean(login),
					login
						? `token da conta ${login}`
						: "o token não foi aceito pelo GitHub",
				);
				...
				if (repoRes.ok) {
					...
					const isPersonal =
						login.toLowerCase() === input.owner.trim().toLowerCase();
					const ownerRes = isPersonal
						? null
						: await api(`/orgs/${input.owner}`);
		...
		async ensureRepo(options) {
			try {
				...
				if (existing.ok) {
					if (!options?.initialProvision)
						return { ok: true, created: false, via: "existing" };
					...
					const files = (treeBody.tree ?? []).filter(
						(entry) => entry.type === "blob",
					);
					const onlyReadme =
						files.length === 1 && files[0]?.path === "README.md";
					let knownSeed = false;
					if (onlyReadme) {
						const content = await api(
							`/repos/${target}/contents/README.md?ref=${branch}`,
						);
						...
						const decoded =
							contentBody.encoding === "base64" && contentBody.content
								? new TextDecoder().decode(
										Uint8Array.from(
											atob(contentBody.content.replace(/\s+/g, "")),
											(c) => c.charCodeAt(0),
										),
									)
								: "";
					...
					for (let suffix = 1; suffix <= 20; suffix += 1) {
						const candidate =
							suffix === 1 ? backupBase : `${backupBase}-${suffix}`;
						const candidateRes = await api(
							`/repos/${input.owner}/${candidate}`,
						);
						...
						if (!candidateRes.ok) {
							return {
								...
								error: await fail(
									candidateRes,
									`verificar o nome de backup ${candidate}`,
								),
							};
						}
						if (suffix === 20) {
					...
					if (!renamed.ok) {
						...
						if (renamed.status !== 403) {
							return {
								...
								error: await fail(
									renamed,
									`preservar o repositório técnico ${target} como backup`,
								),
							};
						}
						...
						for (let suffix = 1; suffix <= 20; suffix += 1) {
							...
							if (!candidateRes.ok) {
								return {
									...
									error: await fail(
										candidateRes,
										`verificar o novo repositório ${candidate}`,
									),
								};
							}
						}
		...
		async masterHeadSha() {
			try {
				...
				if (!body.sha)
					return { ok: false, error: "commit do MASTER não retornado" };
		...
		async releaseSnapshotAtCommit(sha) {
			try {
				const [versionFile, deltaFile, manifestFile] = await Promise.all([
					readMasterFileAtCommit(
						"supabase/baseline-snapshot/tools/delta_version.txt",
						sha,
					),
					readMasterFileAtCommit(
						"supabase/baseline-snapshot/007_delta_migrations.sql",
						sha,
					),
					readMasterFileAtCommit(
				...
				const version = /^\s*version\s*=\s*(\S+)\s*$/m.exec(
					versionFile.content,
				)?.[1];
				...
				const manifestCheck = await validateDeltaManifest(
		...
		async installedRelease() {
			try {
				...
				if (!headBody.sha)
					return { ok: false, error: "commit da instalação não retornado" };
				const path = "supabase/baseline-snapshot/tools/delta_version.txt";
				const res = await api(
				...
				if (!match?.[1]) {
					return {
						ok: false,
						error:
							"versão do pacote não encontrada no repositório da instalação",
					};
				}
		...
		async nudgeDeploy(message) {
			...
			try {
				...
				if (!headSha)
					return { ok: false, error: "HEAD do destino não retornado" };
				...
				const created = await api(`/repos/${target}/git/commits`, {
					...
					body: JSON.stringify({
						message:
							(message ?? "").trim() ||
							"chore(unitos): republicar com variáveis atualizadas",
						tree: treeSha,
						parents: [headSha],
					}),
				});
				...
				const newSha = (
					(await created.json().catch(() => ({}))) as { sha?: string }
				).sha;
				if (!newSha)
					return { ok: false, error: "commit de publicação não retornado" };
			...
			try {
				...
				const readHead = async () => {
					...
					if (res.status === 404 || res.status === 409)
						return { ok: true as const, sha: null };
					return {
						ok: false as const,
				...
				const current = new Map(
					destination.entries.map((e) => [e.path ?? "", e.sha ?? ""]),
				);

				const changed = source.entries.filter(
					(e) => current.get(e.path ?? "") !== e.sha,
				);
						...
						const worker = async () => {
							for (;;) {
								...
								const blob = await api(
									`/repos/${master}/git/blobs/${file.sha}`,
									undefined,
									true,
								);
								if (!blob.ok) {
								...
								if (!created.ok) {
									batchError = await fail(
										created,
										`publicar ${file.path} em ${target}`,
									);
									return;
								}
				...
				if (sharedObjects) {
					await notify(
						80,
						`${changed.length} arquivos reaproveitados do MASTER`,
					);
					entries = [];
				} else {
				...
				if (!treeSha)
					return { ok: false, error: `árvore de ${target} não retornada` };
				...
				if (!update.ok) {
					return {
						...
						error: await fail(
							update,
							`atualizar a branch ${branch} de ${target}`,
						),
					};
				}
				return {
...
export function createDeployClient(input: {
	...
	const projectPath = () =>
		encodeURIComponent(resolvedProjectId ?? resolvedProjectName);
	...
	const fetchProject = async (): Promise<Response> => {
		const request = (teamId: string | null) => {
			...
			return doFetch(
				`https://api.vercel.com/v9/projects/${projectPath()}${suffix}`,
				{ headers },
			);
		};
		const initial = await request(resolvedTeamId);
		if (initial.ok || (initial.status !== 403 && initial.status !== 404))
			return initial;
		...
		const discoverEquivalent = async (
			teamId: string | null,
		): Promise<Response | null> => {
			const suffix = teamId ? `&teamId=${encodeURIComponent(teamId)}` : "";
			const list = await doFetch(
				`https://api.vercel.com/v9/projects?limit=100${suffix}`,
				{
					headers,
				},
			).catch(() => null);
	...
	const client: DeployClient = {
			...
			try {
				...
				if (!checkpoint?.teamId && !resolvedTeamId) {
					...
					if (existing.ok) {
						...
						const linkConfirmation = await confirmProjectRepository(
							body,
							projectId,
							expectedRepo,
						);
				...
				const requestedProject = (
					checkpoint?.projectId ?? input.project
				).trim();
				...
				if (!projectId)
					return { ok: false, error: "a Vercel não retornou o ID do projeto" };
		...
		async setAutoDeploy(enabled) {
			...
			try {
				...
				if (
					res.status === 403 &&
					/pro_plan_required|not available for Hobby/i.test(text)
				) {
					return {
						ok: true,
						unsupported: true,
						error:
							"plano da Vercel não permite política de deployment (auto-deploy segue ligado)",
					};
				}
				return {
					ok: false,
		...
		async linkRepository(repo, options) {
			...
			try {
				...
				const confirmation = await confirmProjectRepository(
					body,
					body.id ?? input.project,
					slug,
				);
			...
			try {
				...
				const publicResponse =
					...
					authenticated.status === 404
						...
						: null;
				const res = publicResponse?.ok
					? publicResponse
					: (authenticated ?? publicResponse);
				if (!res)
					return { ok: false, error: "resposta do GitHub não retornada" };
				if (!res.ok) {
					const detail = (await res.text().catch(() => "")).slice(0, 200);
					const hint =
						res.status === 403 || res.status === 404
		...
		async ensureDomain(domain) {
			...
			try {
				...
				if (!created.ok) {
					...
					const pending =
						created.status === 409 ||
						/domain_already_in_use|domain.*already.*use/i.test(text);
					return {
						ok: false,
						pending,
		...
		async deploymentState(id) {
			try {
				...
				const refused =
					state === "BLOCKED" || body.alwaysRefuseToBuild === true;
				return {
					ok: true,
					...(state ? { state } : {}),
			...
			try {
				...
				const deployment = (body.deployments ?? [])
					.filter((candidate) => {
						...
						return (
							actual === expected && candidate.source?.toLowerCase() === "git"
						);
					})
					.sort((left, right) => {
		...
		async listEnv(plainKeys = []) {
			try {
				...
				const keys = Array.from(
					new Set(envs.map((e) => e.key ?? "").filter(Boolean)),
				);
...
export async function readBaselineProgress(
	...
	for (const row of rows) {
		const raw = (row?.detail as { baselineProgress?: unknown } | null)
			?.baselineProgress;
...
export async function saveBaselineProgress(
	...
	if (!fresh)
		throw new Error(
			"Operação não encontrada durante o checkpoint do baseline.",
		);
	const existing =
		(fresh.detail as { baselineProgress?: BaselineProgress } | null)
			?.baselineProgress ?? {};
	...
	const { data: saved, error } = await rpc.rpc(
	...
	if (error || saved !== true)
		throw new Error(error?.message ?? "lease da operação perdida");
}
...
export async function readStageProgress(
	...
	if (error) throw error;
	if (!data)
		throw new Error("Operação não encontrada durante a leitura de etapa.");
	const raw = (data.detail as { stageProgress?: unknown } | null | undefined)
		?.stageProgress;
...
export async function saveStageProgress(
	...
	if (readError) throw readError;
	if (!fresh)
		throw new Error("Operação não encontrada durante o checkpoint de etapa.");
...
export function classifyAccessFailure(
	detail: string,
): "permission" | "transient" | "other" {
	const text = (detail ?? "").trim();
	if (
		/HTTP 401|HTTP 403|rate limit|não acessa o projeto|privileges/i.test(text)
	)
		return "permission";
	if (
		/HTTP 429|HTTP 50[234]|Instabilidade tempor|limitando as chamadas|timeout/i.test(
			text,
		)
	)
		return "transient";
...
export function withRepoWriteHint(detail: string, repoSlug: string): string {
	const text = (detail ?? "").trim();
	if (
		!/HTTP 403|not accessible by personal access token|Resource not accessible/i.test(
			text,
		)
	) {
		return text;
...
export async function preflightAccess(input: {
	...
	if (input.management) {
		...
		if (ping.ok) {
			...
			const ok =
				keys.ok && Boolean(keys.publishableKey) && Boolean(keys.serviceRoleKey);
			note(
				"database",
				...
				ok
					...
					: (keys.error ??
							"o token não permite ler todas as chaves de API do projeto"),
			);
		}
	}
...
export async function runAutomatedProvision(input: {
	...
	const finish = async (
		...
		await finalizeOperation(client as never, operation as never, {
			...
			errorKind:
				outcome.result === "PASS" ? null : outcome.result.toLowerCase(),
	...
	const mark = async (
		...
		if (state !== "running")
			steps.push({ id, state, detail: sanitize(detail ?? null) });
	...
	const managementToken = (
		env["UNITOS_SUPABASE_MANAGEMENT_TOKEN"] ?? ""
	).trim();
	...
	const masterGithubToken =
		(process.env["UNITOS_GITHUB_TOKEN"] ?? "").trim() || githubToken;
	...
	if (!ping.ok) {
		...
		const transient =
			/Instabilidade tempor|limitando as chamadas|timeout/i.test(detail);
		blocked.push(
			transient
				? detail
	...
	if (!pingRow || pingRow.schemas === undefined) {
		blocked.push(
			"Supabase respondeu sem o resultado esperado da verificação de schemas.",
		);
	...
	if (schemas < 3) {
		blocked.push(
			"O alvo não é um projeto Supabase completo (auth/storage/vault ausentes).",
		);
	...
	const suppliedPublishable = (
		env["UNITOS_SUPABASE_PUBLISHABLE_KEY"] ?? ""
	).trim();
	const suppliedServiceRole = (
		env["UNITOS_SUPABASE_SERVICE_ROLE_KEY"] ?? ""
	).trim();
	const resolvedKeys =
	...
	if (codeStage.codeDone && codeStage.codeSha) {
		...
		if (!ensured.ok) {
			blocked.push(
				`Repositório da instalação indisponível: ${ensured.error ?? ""}`.trim(),
			);
		...
		if (effectiveRepoSlug !== repo.slug) {
			...
			if (updated.error) {
				...
				await mark(
					"code",
					"error",
					updated.error.message ?? "falha ao atualizar repositório",
				);
				checks.code = "error";
				return finish(null, null);
			}
		...
		const { compareReleaseVersions, masterNotPublishedMessage } = await import(
			"./manager-contract"
		);
		if (
			compareReleaseVersions(sourceRelease.version, MASTER_RELEASE_VERSION) < 0
		) {
			const reason = masterNotPublishedMessage(
		...
		if (sourceRelease.version !== installedRelease.version) {
			...
			const reusable =
				stage.codeSourceSha === masterHead.sha ? (stage.codeBlobs ?? {}) : {};
			...
			if (!published.ok) {
				const detail = withRepoWriteHint(
					published.error ??
						`não foi possível sincronizar ${effectiveRepoSlug}`,
					effectiveRepoSlug,
				);
				const kind = classifyAccessFailure(detail);
				if (kind === "permission")
					blocked.push(`Código não sincronizado: ${detail}`);
	...
	if (
		...
		const reason =
			ensuredProject.error ??
			"projeto Vercel não pôde ser criado ou localizado";
	...
	if (!repositoryLinked) {
		...
		if (!linked.ok) {
			...
			await mark(
				"deploy_link",
				"error",
				linked.error ?? "vínculo do repositório falhou",
			);
			checks.configuration = "attention";
			return finish(null, null);
		}
		if (ensuredProject.teamId !== "personal") {
			...
			if (!confirmed.ok || confirmed.repositoryLinked !== true) {
				const reason =
					confirmed.error ??
					"vínculo GitHub/branch main não foi confirmado pela Vercel";
	...
	if (!autoDeployOn.ok) {
		blocked.push(
			`Auto-deploy por Git não confirmado: ${autoDeployOn.error ?? "falha"}`,
		);
	...
	const runBaselinePhase = async (
		...
		const progress = await readBaselineProgress(
			client,
			installation.id,
			operation,
		);
		...
		const groupPercent = (id: string, fileFraction: number) =>
			Math.min(
				99,
				Math.round(
					(((groupDone[id] ?? 0) + fileFraction) /
						Math.max(groupTotals[id] ?? 1, 1)) *
						100,
				),
			);
		...
		for (const file of baseline) {
			...
			if (progress[file.key] === DONE) {
				if (file.key === "000_extensions") {
					const vectorReady =
						await verifyVectorExtensionPostcondition(management);
					if (!vectorReady.ok) {
			...
			if (file.label === UPDATE_DELTA_LABEL) {
				...
				if (
					...
					await mark(
						"database",
						"error",
						"delta legado parcial sem evidência por migration",
					);
					checks.database = "error";
					return finish(appUrl, urlSource);
				}
				if (!provisionCommitSha) {
					blocked.push(
						"commit fixado do MASTER ausente para executar migrations",
					);
					await mark("database", "error", "pacote do MASTER não foi fixado");
					checks.database = "error";
					return finish(appUrl, urlSource);
				}
				const snapshot = localDeltaPackage(
					provisionCommitSha,
					operation.baseline_id,
				);
				if (!snapshot.version || !snapshot.sha256 || snapshot.total === 0) {
					blocked.push(
						"metadados locais do pacote de migrations estão incompletos ou divergentes",
					);
				...
				if (!operation.baseline_id && !operation.baseline_hash) {
					...
					if (sealError || sealed !== true) {
						failures.push(
							sealError?.message ??
								"não foi possível fixar o pacote da instalação nova",
						);
			...
			const perStatement = await applyStatementByStatement(
				management,
				prepared.sql,
				{
					isCancelled,
					...
					onProgress: async (processed, total) => {
						...
						const action =
							alreadyApplied > 0 ? "retomando aplicação" : "aplicando";
						await mark(
							file.id,
							"running",
			...
			if (!perStatement.ok) {
				if (
					typeof perStatement.processed === "number" &&
					perStatement.processed > 0
				) {
					progress[file.key] = perStatement.processed;
					await saveBaselineProgress(client, operation, progress);
				}
				failures.push(
					`${file.label}: ${perStatement.error ?? "falha ao aplicar"}`,
				);
				await mark(file.id, "error", `${file.label} falhou`);
				checks[file.id === "seeds" ? "database" : (file.id as HealthCheckId)] =
					"error";
				return finish(appUrl, urlSource);
			}
	...
	if (
		...
		await mark(
			"secrets",
			"done",
			"secrets próprios já gerados nesta operação (checkpoint)",
		);
		await mark(
			"deploy",
			"done",
		...
		try {
			...
			const reason =
				error instanceof Error ? error.message : "falha ao preparar as chaves";
		...
		const vault = await management.query(
			writeCronSecretToVaultSql(secrets.CRON_SECRET),
		);
		if (!vault.ok) {
			failures.push(
		...
		if (containsMasterReference(resolved.origin)) {
			failures.push(
				"A URL resolvida aponta para o MASTER — operação recusada.",
			);
		...
		try {
			...
			if (creds.appType === "unitos" && creds.appId && creds.appSecret) {
				officialMetaApp = {
					...
					businessConfigId:
						creds.businessConfigId ?? (await resolveMetaBusinessConfigId()),
				};
			}
		} catch {
		...
		if (!autoDeploy.ok) {
			blocked.push(
				`Auto-deploy por Git não confirmado: ${autoDeploy.error ?? "falha"}`,
			);
		...
		if (!envResult.ok) {
			blocked.push(
				`Variáveis do deploy não configuradas: ${envResult.error ?? ""}`.trim(),
			);
		...
		if (!identity.ok) {
			failures.push(
				`installation.app_url não registrada: ${identity.error ?? ""}`.trim(),
			);
		...
		let expectedDeploymentCommit =
			stage.provisionDeploymentCommit ?? provisionCommitSha;
		...
		if (!deploymentId && stage.provisionGitPushCommit) {
			const located = await deploy.findProductionDeployment(
				stage.provisionGitPushCommit,
			);
			if (!located.ok || !located.deploymentId) {
				...
				await mark(
					...
					"running",
					located.error ??
						"aguardando a hospedagem detectar o commit de publicação pelo Git",
				);
				return {
					result: "RUNNING",
					reasons: [],
		...
		let redeployed: Awaited<ReturnType<DeployClient["deployLatestCode"]>> =
			deploymentId
		...
		if (!redeployed.ok) {
			if (redeployed.quotaExceeded || redeployed.gitSourceUnavailable) {
				...
				if (!fallbackCommit && provisionCommitSha) {
					const existingByCommit =
						await deploy.findProductionDeployment(provisionCommitSha);
					if (!existingByCommit.ok) {
						...
						await mark(
							"deploy",
							"running",
							existingByCommit.error ?? "aguardando a hospedagem",
						);
						return {
							result: "RUNNING",
							reasons: [],
		...
		if (
			deploymentResult.refused ||
			FAILED_DEPLOYMENT_STATES.has(deploymentResult.state)
		) {
		...
		if (deploymentResult.timedOut || deploymentResult.state !== "READY") {
			await mark(
				"deploy",
				"running",
				`deployment em andamento (${deploymentResult.state})`,
			);
			return {
				result: "RUNNING",
				reasons: [],
		...
		const readyProof = validateReadyDeploymentCommit(
			deploymentResult,
		...
		if (!redeployed.ok && failures.length === 0) {
			await mark(
				"deploy",
				"error",
				blocked.join(" | ") || "deployment não confirmado",
			);
			checks.frontend = "error";
			return finish(url.origin, url.source);
		}
	...
	if (!hardened.ok) {
		failures.push(
			`verify-installation: reparo das filas falhou: ${hardened.error ?? "falha"}`,
		);
	...
	if (!summary.ok) {
		failures.push(
			`verify-installation: ${summary.reason ?? "resultado inconclusivo"}`,
		);
...
export async function readFirstAccessState(management: {
	...
	const workspace: CheckState =
		brands === 1 ? "ok" : brands === 0 ? "attention" : "error";
	return {
...
export function classifyVerificationTarget(
	...
	if (name.startsWith("storage:"))
		return { step: "storage", health: "storage" };
	if (
		name.startsWith("cron:") ||
		name.startsWith("vault:") ||
		name.includes("brain_stats_mv")
	)
...
export function classifyVerificationCheck(
	checkName: string,
): VerificationTarget["step"] {
	return classifyVerificationTarget(checkName).step;
}

export function classifyVerificationHealthCheck(
	checkName: string,
): HealthCheckId {
...
export async function runAutomatedValidate(input: {
	...
	const capability = resolveAutomationCapability(env);
	if (!capability.available)
		return fail("BLOCKED", capability.blockedReasons.join(" | "));
...
function localDeltaPackage(
	...
	const currentVersion =
		/^version=(.+)$/m.exec(deltaVersion)?.[1]?.trim() ?? "";
	...
	const sealedCommit =
		sealed.length >= 3 ? sealed.slice(1, -1).join(":") : null;
...
export function readAppliedMigrationLabels(
	...
	for (const row of rows) {
		...
		const explicit =
			row.file && row.fingerprint
				? `${String(row.file)}:${String(row.fingerprint)}`
				: "";
...
export function needsLegacyBlobReconciliation(
	...
	const highestApplied = migrations.reduce(
		(highest, migration, index) =>
			appliedLabels.has(`${migration.file}:${migration.fingerprint}`)
				? index
				: highest,
		-1,
	);
...
export function databaseMigrationsPercent(input: {
	...
	const currentFraction =
		input.currentTotal && input.currentTotal > 0
			? Math.min(
					1,
					Math.max(0, (input.currentProcessed ?? 0) / input.currentTotal),
				)
			: 0;
	return Math.min(
		99,
		Math.max(
			0,
			Math.round(((input.completed + currentFraction) / input.total) * 100),
		),
	);
}
...
export function attachCanonicalMigrationIdentity(
	...
	if (entries.length !== migrations.length) {
		throw new Error(
			"Manifesto canônico não cobre o inventário integral de migrations.",
		);
	}
...
export async function recoverLegacyCanonicalManifest(
	...
	if (
		...
		return {
			ok: false,
			error:
				"manifesto canônico ausente e pacote legado diverge do contrato oficial do MASTER",
		};
	}
...
export function operationPackageIdentity(
	snapshot: Omit<OperationPackageSnapshot, "sql">,
): string {
...
export async function validateCanonicalPackage(
	...
	const digest = await crypto.subtle.digest(
		"SHA-256",
		new TextEncoder().encode(snapshot.sql),
	);
	const actualSha = Array.from(new Uint8Array(digest))
		.map((byte) => byte.toString(16).padStart(2, "0"))
		.join("");
...
export function reconcileConfirmedMigrationCount(
	confirmed: number,
	observed: number,
) {
...
export function validateCanonicalMigrationProgress(
	...
	for (const row of rows) {
		...
		if (
			...
			throw new Error(
				"Progresso canônico parcial ou divergente do pacote fixado.",
			);
		}
		if (row.status === "completed") {
			if (current || row.statement_index !== row.total_statements) {
				throw new Error(
					"Progresso canônico contém uma conclusão incompatível.",
				);
			}
			...
			if (current)
				throw new Error(
					"Progresso canônico contém mais de uma migration em execução.",
				);
			current = row;
		}
...
async function readCanonicalMigrationProgress(
	...
	if (error)
		throw new Error(error.message ?? "falha ao ler progresso canônico");
	if (!Array.isArray(data))
		throw new Error(
			"Leitura do progresso canônico retornou resposta inválida.",
		);
	return data.filter((row): row is CanonicalMigrationProgress => {
		if (!row || typeof row !== "object") return false;
		const value = row as Record<string, unknown>;
		return (
			typeof value["migration_file"] === "string" &&
...
async function checkpointCanonicalMigration(
	...
	if (error || data !== true)
		throw new Error(
			error?.message ?? "lease da operação perdida no checkpoint canônico",
		);
}
...
async function reconcileLegacyMigrationMarker(
	...
	{ ok: false; detail: string } | { ok: true; promotions: LegacyPromotion[] }
> {
	const inspection = await management.query(
	...
	const storedResponse = await rpc.rpc(
		...
		return {
			...
			detail:
				cause instanceof Error
					? cause.message
					: "Evidências persistidas inválidas",
		};
	}
...
> {
	...
	const validatedSnapshot = await validateCanonicalPackage(
		operation,
		input.snapshot,
	);
	if (!validatedSnapshot.ok)
		return { state: "blocked", detail: validatedSnapshot.error };
	const recoveredManifest = await recoverLegacyCanonicalManifest(
		input.snapshot,
	);
	if (!recoveredManifest.ok)
		return { state: "blocked", detail: recoveredManifest.error };
	try {
		migrations = attachCanonicalMigrationIdentity(
			migrations,
			recoveredManifest.manifest,
		);
	} catch (cause) {
		return {
			state: "blocked",
			detail:
				cause instanceof Error ? cause.message : "inventário canônico inválido",
		};
	}
	...
	await reconcileCanonicalMigrations(
		client,
		operation,
		migrations,
		appliedLabels,
	);
	let canonicalProgress = await readCanonicalMigrationProgress(
		client,
		operation,
	);
	assertCompletedProgressBackedByClientLedger(canonicalProgress, appliedLabels);
	let canonicalState = validateCanonicalMigrationProgress(
		canonicalProgress,
		migrations,
	);
	...
	const migrationLimit =
		input.maxMigrationsPerInvocation ??
		UPDATE_DATABASE_MIGRATIONS_PER_INVOCATION;
	...
	while (migration) {
		...
		const migrationPosition =
			migrations.findIndex(
				(item) =>
					item.file === migration?.file &&
					item.fingerprint === migration?.fingerprint,
			) + 1;
		...
		const applied = await applyStatementByStatement(management, prepared.sql, {
			...
			maxStatements:
				input.maxStatementsPerInvocation ?? BASELINE_STATEMENTS_PER_INVOCATION,
		});
		...
		if (
			processedMigrations >= migrationLimit ||
		...
		canonicalState = validateCanonicalMigrationProgress(
			canonicalProgress,
			migrations,
		);
		migration = migrations.find(
			(item) =>
				!canonicalState.completed.has(`${item.file}:${item.fingerprint}`),
		);
	}
...
export async function runAutomatedUpdate(input: {
	...
	if (!capability.vercel.available) {
		return fail(
			"BLOCKED",
			capability.vercel.reason ?? "token de deploy indisponível no MASTER",
		);
	}
	...
	if (!project) {
		return fail(
			"BLOCKED",
			"a instalação não tem projeto de deploy configurado",
		);
	}
	...
	if (!capability.github.available) {
		return fail(
			"BLOCKED",
			capability.github.reason ?? "token do GitHub indisponível no MASTER",
		);
	}
	...
	const alreadyPublished =
		checkpoint.codeDone === true && Boolean(checkpoint.codeSha);
	...
	if (
		...
		return fail(
			"BLOCKED",
			packageSnapshot.error ?? "snapshot autorizado do MASTER indisponível",
		);
	}
	const snapshot = {
	...
	if (!operation.baseline_id && !operation.baseline_hash) {
		...
		if (sealError || sealed !== true) {
			return fail(
				"BLOCKED",
				sealError?.message ??
					"não foi possível fixar o pacote autorizado da operação",
			);
		}
		operation.baseline_id = operationPackageIdentity(snapshot);
		operation.baseline_hash = snapshot.sha256;
	}
	const snapshotValidation = validateOperationPackageSnapshot(
		operation,
		snapshot,
	);
	...
	if (delta.state === "blocked" || delta.state === "error") {
		return fail(
			delta.state === "blocked" ? "BLOCKED" : "FAIL",
			delta.detail,
			"database",
		);
	}
	...
	let deploymentId = checkpoint.updateGitPushCommit
		...
		: null;
	let deploymentSource: "git" | "rebuild" | undefined =
		checkpoint.updateGitPushCommit ? "git" : undefined;
	let deploymentRef =
		checkpoint.updateGitPushCommit ?? checkpoint.updateDeploymentRef;
	...
	if (!publishedRelease) {
		...
		if (!repoRelease.ok || !repoRelease.version) {
			return fail(
				"BLOCKED",
				repoRelease.error ??
					"versão do pacote do MASTER não pôde ser lida no commit autorizado",
				"code",
			);
		}
	...
	if (!deploymentId) {
		...
		if (!ensured.ok) {
			return fail(
				"BLOCKED",
				ensured.error ?? `repositório ${repo.slug} indisponível`,
			);
		}
		const reusableBlobs =
			checkpoint.codeSourceSha === targetSha
				? (checkpoint.codeBlobs ?? {})
				: {};
		await saveStageProgress(client, operation, {
		...
		if (!published.ok) {
			...
			return fail(
				classifyAccessFailure(detail) === "permission" ? "BLOCKED" : "FAIL",
				detail,
			);
		}
		...
		changedFiles =
			typeof published.changed === "number" ? published.changed : null;
		await saveStageProgress(client, operation, {
			codeDone: true,
			codeSha: targetSha,
			codeRepo: repo.slug,
	...
	const finishByGitPush = async (
		...
		if (!pushedCommit) {
			const nudge = await code.nudgeDeploy(
				"chore(unitos): republicar versao autorizada",
			);
			if (!nudge.ok || !nudge.commitSha) {
				return fail(
					"FAIL",
		...
		await report(
			...
			"code",
			"done",
			"código publicado no repositório",
		);
		await report(
			client,
			operation,
			"build",
			"running",
			`build disparado pelo Git (${cause})`,
		);
		...
		if (FAILED_DEPLOYMENT_STATES.has(pushState)) {
			return fail(
				"FAIL",
				`o build disparado pelo Git terminou em ${pushState}`,
				"build",
			);
		}
		if (pushState !== "READY") {
			...
			const elapsedMin = Number.isFinite(startedAt)
		...
		await hardenHelperTables(management);
		const pushVerification = await management.query(
			prepareVerificationSql(verifySql).sql,
		);
