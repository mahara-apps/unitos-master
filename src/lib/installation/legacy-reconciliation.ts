...
const CONTRACT = [
  ...
  [34, "canonical_state"],
  [39, "canonical_state"],
  [42, "partial_compatibility"],
  [52, "partial_compatibility"],
  [55, "canonical_state"],
  [56, "partial_compatibility"],
  [61, "canonical_state"],
  [64, "canonical_state"],
  [66, "partial_compatibility"],
  [70, "canonical_state"],
  [71, "canonical_state"],
  [72, "external_checkpoint_required"],
  [74, "external_checkpoint_required"],
  [82, "partial_compatibility"],
  [83, "partial_compatibility"],
  [84, "canonical_state"],
...
function functionCondition(input: {
  ...
  if (input.securityDefiner !== undefined) {
    clauses.push(
      `(SELECT prosecdef = ${input.securityDefiner ? "true" : "false"} FROM pg_proc WHERE oid=${oid})`,
    );
  }
...
function resultSelect(
  ...
  const status = condition
    ? `CASE WHEN (${condition}) THEN 'compatible' ELSE 'divergent' END`
    : `'insufficient'`;
  const observed = condition
    ? `CASE WHEN (${condition}) THEN 'pós-condição canônica confirmada' ELSE 'pós-condição ausente ou divergente' END`
    : `'checkpoint externo canônico ausente'`;
...
export function buildLegacyReconciliationInspectionSql(migrations: Migration[]): string {
  ...
  const conditions = new Map<number, string | null>([
    ...
    [
      84,
      `${dangerousAcl} AND ${defaultAcl} AND has_schema_privilege('anon','public','USAGE') AND has_table_privilege('anon','public.installation','SELECT')`,
    ],
    [85, null],
  ]);
...
export function normalizeLegacyEvidenceRows(rows: unknown[]): LegacyEvidenceResult[] {
  ...
  for (const raw of rows) {
    if (!raw || typeof raw !== "object")
      throw new Error("Resposta de reconciliação contém linha inválida.");
  ...
  if (results.length !== CONTRACT.length)
    throw new Error("Resposta de reconciliação está incompleta.");
...
export function legacyEvidenceBlockReason(results: LegacyEvidenceResult[]): string | null {
  ...
  if (divergent.length)
    return `reconciliação legada divergente nas posições ${divergent.map((item) => item.position).join(", ")}`;
  const automaticMissing = results.filter(
    (item) => item.classification === "canonical_state" && item.status !== "compatible",
  );
...
export function reconciledLegacyPositions(results: LegacyEvidenceResult[]): Set<number> {
  return new Set(
    results
      .filter(
        (item) => item.status === "compatible" && item.classification !== "partial_compatibility",
      )
