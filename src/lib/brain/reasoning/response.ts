...
function renderTasks(q: string, r: ToolResult[]): string {
  const overdue = rowsOf<{ title: string; due_at: string | null; assignee_id: string | null; project_id: string | null }>(r, "tasks.overdue");
  const counts = pickOf<{ total: number; done: number; overdue: number; open: number }>(r, "tasks.count");
  ...
  if (/atrasad|vencid|em atraso/i.test(q) || overdue.length > 0) {
    ...
    if (dias !== null) lines.push(`A mais antiga (“${first.title}”) está vencida há ${dias} dia(s).`);
...
function renderProjects(r: ToolResult[]): string {
  const list = rowsOf<{ name: string; status: string; progress: number | null; due_at: string | null }>(r, "projects.list");
  const buckets = pickOf<Record<string, number>>(r, "projects.status") ?? {};
  if (!list.length && !Object.keys(buckets).length) return "Nenhum projeto encontrado no workspace.";
  const status = Object.entries(buckets).map(([k, v]) => `${v} ${k}`).join(" · ");
  const lines = [`**${list.length} projeto(s)** — ${status || "sem status"}.`];
  for (const p of list.slice(0, 5)) {
    lines.push(`- ${p.name} · ${p.status}${p.progress != null ? ` · ${p.progress}%` : ""}${p.due_at ? ` · entrega ${fmtDate(p.due_at)}` : ""}`);
  }
  return lines.join("\n");
}
...
function renderContent(r: ToolResult[]): string {
  ...
  const upcoming = rowsOf<{ title: string; scheduled_at: string | null; stage: string }>(r, "content.upcoming");
  if (!Object.keys(counts).length && !upcoming.length) return "Nenhum conteúdo cadastrado no workspace.";
  const total = Object.values(counts).reduce((a, b) => a + b, 0);
  const byStage = Object.entries(counts).map(([k, v]) => `${v} em ${k}`).join(", ");
...
function renderCalendar(r: ToolResult[]): string {
  const data = pickOf<{ posts: Array<{ title: string; scheduled_at: string | null }>; tasks: Array<{ title: string; due_at: string | null }> }>(r, "calendar.upcoming");
  ...
  if (data.posts.length) {
    ...
    for (const p of data.posts.slice(0, 5)) lines.push(`- ${p.title || "(sem título)"} · ${fmtDate(p.scheduled_at)}`);
  }
...
function renderMetrics(r: ToolResult[]): string {
  ...
  if (counts) lines.push(`- Tarefas: ${counts.total} (${counts.overdue} em atraso, ${counts.done} concluídas)`);
...
function renderSummary(r: ToolResult[]): string {
  ...
  if (stats) parts.push(`Operação: ${Object.entries(stats).map(([k, v]) => `${v} ${k}`).join(" · ")}.`);
  if (overdue.length) parts.push(`Atenção: ${overdue.length} tarefa(s) em atraso.`);
  if (upcoming.length) parts.push(`Próxima publicação: “${upcoming[0].title || "(sem título)"}” em ${fmtDate(upcoming[0].scheduled_at)}.`);
...
function renderRecommendations(r: ToolResult[]): string {
  ...
  for (const rec of recs.slice(0, 6)) lines.push(`- ${rec.title ?? rec.description ?? "(sem título)"}`);
  return lines.join("\n");
}
