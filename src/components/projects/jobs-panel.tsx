...
import { JobTaskRow } from "./job-task-row";
...
export function JobsPanel({ brandId, projectId, projectName = "Projeto", clientName, team = [], currentUserId, pautasContent, pautasCount = 0, footer, onOpenPautas, onCreatePauta, initialMode = "overview", initialJobId = null, onOpenJobChange }: Props) {
  ...
  const taskStatusMap = useMemo(() => new Map((taskStatusesQ.data ?? []).map((status) => [status.id, status])), [taskStatusesQ.data]);
  ...
  return (
    <>
      <DashboardPanelSurface className="overflow-hidden">
        <div className="border-b border-border/60 bg-background/40 p-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <Button variant="outline" className={cn("h-auto justify-between rounded-md px-4 py-3 text-left", mode === "jobs" && "border-primary/30 bg-primary/5 hover:bg-primary/5")} onClick={() => setMode("jobs")}><span className="flex min-w-0 items-center gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-md bg-primary/10 text-primary"><List className="h-4 w-4" /></span><span><span className="block text-sm font-bold">Jobs</span><span className="block text-[11px] font-medium text-muted-foreground">{jobs.length} frentes de trabalho · {taskTotals.total} tarefas</span></span></span></Button>
            {hasPautas ? <Button variant="outline" className="h-auto justify-between rounded-md px-4 py-3 text-left" onClick={openPautas}><span className="flex min-w-0 items-center gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-md bg-accent text-accent-foreground"><Sparkles className="h-4 w-4" /></span><span><span className="block text-sm font-bold">Pautas</span><span className="block text-[11px] font-medium text-muted-foreground">{pautasCount} {pautasCount === 1 ? "peça" : "peças"} de conteúdo</span></span></span><ChevronRight className="h-4 w-4 shrink-0 text-muted-foreground" /></Button> : null}
          </div>
        </div>
      ...
      <JobDetailModal
        ...
        controls={currentJob ? <><div className="min-w-0"><span className="mb-1 block text-[9px] font-medium uppercase text-muted-foreground">Responsável</span><AssigneePicker value={currentJob.assignee_id} options={team} className="h-8 w-full" onChange={(assigneeId) => patchJobMut.mutate({ jobId: currentJob.id, patch: { assignee_id: assigneeId } })} /></div><div className="min-w-0"><span className="mb-1 block text-[9px] font-medium uppercase text-muted-foreground">Status</span><StatusPicker brandId={brandId} scope="job" value={currentJob.status_id} className="h-8 w-full" onChange={(statusId) => patchJobMut.mutate({ jobId: currentJob.id, patch: { status_id: statusId } })} /></div><div><span className="mb-1 block text-[9px] font-medium uppercase text-muted-foreground">Prazo</span><DueDateChip value={currentJob.due_at} overdue={isOverdue(currentJob.due_at, !!currentJob.done_at)} onChange={(dueAt) => patchJobMut.mutate({ jobId: currentJob.id, patch: { due_at: dueAt } })} /></div><div><span className="mb-1 block text-[9px] font-medium uppercase text-muted-foreground">Tempo</span><JobTimerWidget brandId={brandId} jobId={currentJob.id} estimatedMinutes={currentJob.estimated_minutes} /></div></> : null}
        ...
        timeline={currentJob ? <div className="grid grid-cols-[120px_minmax(0,1fr)_120px] items-center gap-3"><Input type="date" className="h-8 w-full text-xs" aria-label="Início do job" defaultValue={dateOnly(currentJob.start_date) ?? ""} onBlur={(event) => patchJobMut.mutate({ jobId: currentJob.id, patch: { start_date: event.target.value || null } })} /><div className="min-w-0"><div className="mb-1.5 flex items-center justify-between gap-2 text-[10px]"><span className="text-muted-foreground">Período</span><span className={cn("truncate font-medium", currentJob.due_at && isOverdue(currentJob.due_at, !!currentJob.done_at) ? "text-destructive" : "text-muted-foreground")}>{timeline.label} · {timeline.percent}%</span></div><div className="relative"><Progress value={timeline.percent} className="h-1.5" /><span className="absolute top-1/2 h-2.5 w-0.5 -translate-y-1/2 bg-foreground" style={{ left: `${timeline.percent}%` }} /></div></div><Input type="date" className="h-8 w-full text-xs" aria-label="Entrega do job" defaultValue={dateOnly(currentJob.due_at) ?? ""} onBlur={(event) => patchJobMut.mutate({ jobId: currentJob.id, patch: { due_at: event.target.value || null } })} /></div> : null}
        main={<div className="@container pb-6"><div className="grid gap-2 border-b border-border/60 px-4 py-3 @[820px]:grid-cols-[minmax(0,1fr)_auto] @[820px]:items-center"><div className="min-w-0"><span className="font-mono text-[11px] uppercase tracking-widest text-muted-foreground">Tarefas</span><span className="ml-2 text-[11px] tabular-nums text-muted-foreground">Abertas {openTasksCount}/{currentJobTasks.length} · {formatMinutes(currentStats.minutes)}</span></div><div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-1 @[520px]:flex @[520px]:justify-end"><div className="relative col-span-3 min-w-0 @[520px]:col-auto @[520px]:w-40"><Search className="pointer-events-none absolute left-2 top-1/2 h-3 w-3 -translate-y-1/2 text-muted-foreground" /><Input value={taskSearch} onChange={(event) => setTaskSearch(event.target.value)} placeholder="Buscar tarefas" aria-label="Buscar tarefas" className="h-8 w-full pl-7 text-xs" /></div><Select value={taskSort} onValueChange={(value) => setTaskSort(value as typeof taskSort)}><SelectTrigger className="h-8 w-28 text-xs" aria-label="Ordenar tarefas"><ArrowUpDown className="mr-1 h-3 w-3" /><SelectValue /></SelectTrigger><SelectContent><SelectItem value="position">Ordem</SelectItem><SelectItem value="due">Prazo</SelectItem><SelectItem value="name">Nome</SelectItem></SelectContent></Select><div className="flex shrink-0 rounded-md bg-muted p-0.5"><Button size="sm" variant={taskView === "list" ? "secondary" : "ghost"} className="h-7 gap-1 px-2 text-[11px]" onClick={() => setTaskView("list")}><List className="h-3 w-3" />Lista</Button><Button size="sm" variant={taskView === "board" ? "secondary" : "ghost"} className="h-7 gap-1 px-2 text-[11px]" onClick={() => setTaskView("board")}><Kanban className="h-3 w-3" />Quadro</Button></div><DropdownMenu><DropdownMenuTrigger asChild><Button size="icon" variant="ghost" className="h-8 w-8 shrink-0" aria-label="Filtros de tarefas"><MoreHorizontal className="h-4 w-4" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-52"><VisibilityMenuBlock value={taskVisibility} onChange={setTaskVisibility} label="Exibir tarefas" withSeparator={false} /><DueMenuBlock value={dueFilter} onChange={setDueFilter} label="Prazo" /></DropdownMenuContent></DropdownMenu></div></div>{tasksQ.isLoading ? <div className="space-y-3 p-5"><Skeleton className="h-12" /><Skeleton className="h-12" /></div> : taskView === "board" ? <DndContext sensors={sensors} onDragStart={(event) => setDraggedTaskId(String(event.active.id))} onDragEnd={onTaskDragEnd} onDragCancel={() => setDraggedTaskId(null)}><div className="flex gap-3 overflow-x-auto p-4">{TASK_STATUSES.map((status) => <TaskBoardColumn key={status} status={status} tasks={currentJobTasks.filter((task) => task.status === status)} team={team} statusMap={taskStatusMap} onOpen={setOpenTask} />)}</div><DragOverlay>{draggedTaskId ? <TaskBoardCard task={currentJobTasks.find((task) => task.id === draggedTaskId) ?? currentJobTasks[0]} team={team} status={taskStatusMap.get((currentJobTasks.find((task) => task.id === draggedTaskId) ?? currentJobTasks[0])?.status_id ?? "")} onOpen={() => {}} /> : null}</DragOverlay></DndContext> : currentJobTasks.length === 0 ? <div className="p-10 text-center text-xs text-muted-foreground">Nenhuma tarefa neste job.</div> : <div className="divide-y divide-border/60">{currentJobTasks.map((task) => { const taskStatus = taskStatusMap.get(task.status_id ?? ""); const visualState = resolveWorkItemVisualState({ archivedAt: task.archived_at, done: isItemDone(task), statusName: taskStatus?.name, statusIsDone: taskStatus?.is_done }); return <JobTaskRow key={task.id} title={task.title} done={isItemDone(task)} visualState={visualState} onOpen={() => setOpenTask(task)} completion={<Button size="icon" variant="ghost" className="h-7 w-7 hover:bg-transparent" aria-label={isItemDone(task) ? "Reabrir tarefa" : "Concluir tarefa"} onClick={() => changeTaskStatus(task, isItemDone(task) ? "todo" : "done")}>{visualState === "cancelled" ? <XCircle className="h-4 w-4 text-work-cancelled-foreground" /> : visualState === "archived" ? <Archive className="h-4 w-4 text-work-archived-foreground" /> : isItemDone(task) ? <CheckCircle2 className="h-4 w-4 text-work-done" /> : <span className="h-4 w-4 rounded-full border border-border" />}</Button>} status={<StatusPicker brandId={brandId} scope="task" value={task.status_id} className="h-8 w-full min-w-0 border-dashed text-xs" placeholder="+ status" onChange={(statusId) => changeTaskWorkStatus(task, statusId)} />} priority={<Select value={task.priority} onValueChange={(priority) => patchTaskMut.mutate({ taskId: task.id, patch: { priority: priority as TaskPriority } })}><SelectTrigger className="h-8 w-full min-w-0 text-xs" aria-label="Prioridade"><SelectValue /></SelectTrigger><SelectContent>{TASK_PRIORITIES.map((priority) => <SelectItem key={priority} value={priority}>{PRIORITY_META[priority].label}</SelectItem>)}</SelectContent></Select>} assignee={<AssigneePicker compact value={task.assignee_id} options={team} placeholder="Sem responsável" onChange={(assigneeId) => patchTaskMut.mutate({ taskId: task.id, patch: { assignee_id: assigneeId } })} />} dueDate={<DueDateChip value={task.due_at} overdue={isOverdue(task.due_at, isItemDone(task))} onChange={(dueAt) => patchTaskMut.mutate({ taskId: task.id, patch: { due_at: dueAt } })} />} timer={<TaskTimerWidget brandId={brandId} taskId={task.id} estimatedMinutes={task.estimated_minutes} compact />} subtasks={<TaskSubtasksPopover taskId={task.id} />} menu={taskActions(task)} />; })}</div>}<div className="grid grid-cols-1 gap-2 border-y border-border/60 bg-muted/10 px-4 py-3 sm:grid-cols-[minmax(0,1fr)_135px_auto]"><Input value={newTaskTitle} onChange={(event) => setNewTaskTitle(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && newTaskTitle.trim()) createTaskMut.mutate(); }} placeholder="Adicionar uma tarefa…" className="h-9 min-w-0" /><Input type="date" value={newTaskDue} onChange={(event) => setNewTaskDue(event.target.value)} className="h-9 w-full text-xs" aria-label="Prazo opcional" /><Button size="sm" className="h-9" onClick={() => createTaskMut.mutate()} disabled={!newTaskTitle.trim()}><Plus className="mr-1 h-3.5 w-3.5" />Adicionar</Button></div><JobBriefingEditor key={currentJob?.id} value={currentJob?.description ?? null} onSave={(description) => currentJob ? patchJobMut.mutateAsync({ jobId: currentJob.id, patch: { description } }) : Promise.resolve()} /></div>}
        aside={currentJob ? <ContextTabs className="h-full" tabs={[{ value: "comments", label: "Comentários", content: <CommentThread className="h-full" brandId={brandId} level="job" projectId={projectId} jobId={currentJob.id} currentUserId={currentUserId} placeholder="Insira seu comentário aqui" showHeader={false} /> }, { value: "links", label: "Anexos", content: <WorkLinks target="job" targetId={currentJob.id} title="Anexos e links do job" /> }, { value: "timesheet", label: "Timesheet", content: <JobTimesheetPanel brandId={brandId} jobId={currentJob.id} estimatedMinutes={currentJob.estimated_minutes} /> }, { value: "history", label: "Histórico", content: <JobHistoryPanel brandId={brandId} jobId={currentJob.id} /> }]} /> : null}
      />
