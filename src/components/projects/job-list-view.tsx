...
function JobRow({ brandId, job, stats, rollup, status, team, onOpen, onStatusChange, onAssigneeChange, onDueChange, menu }: {
  ...
  return (
    <div className={cn("grid min-h-[58px] grid-cols-[auto_minmax(150px,1fr)_auto] items-center gap-x-3 gap-y-2 border-b border-l-2 border-border/50 border-l-transparent px-4 py-2.5 transition-colors last:border-b-0 hover:bg-muted/20 lg:grid-cols-[auto_minmax(170px,1fr)_120px_66px_64px_70px_150px_24px]", workItemSurfaceClass(visualState))}>
      ...
      <div className="hidden lg:block"><StatusPicker brandId={brandId} scope="job" value={job.status_id} compactPill selectionOnly className="h-8 w-[150px] rounded-full text-[11px] font-semibold" onChange={onStatusChange} /></div>
      <div className="flex items-center justify-end">{menu}</div>
      <div className="col-span-3 grid grid-cols-[1fr_auto_auto] items-center gap-3 lg:hidden"><ProgressCell stats={stats} color={status?.color} /><div className={cn("flex items-center gap-1 text-xs tabular-nums", rollup?.running ? "font-medium text-work-done" : "text-muted-foreground")}><Clock3 className={cn("h-3.5 w-3.5", rollup?.running && "animate-pulse")} />{formatMinutes(minutes)}</div><JobAvatars job={job} stats={stats} team={team} /><div className="col-span-2"><StatusPicker brandId={brandId} scope="job" value={job.status_id} compactPill selectionOnly className="h-8 w-full max-w-[180px] rounded-full text-[11px] font-semibold" onChange={onStatusChange} /></div><DueDateChip value={job.due_at} overdue={isOverdue(job.due_at, !!job.done_at)} onChange={onDueChange} /></div>
    </div>
  );
}
    ...
    <div>
      <div className="px-4 py-3">
        <div className="flex flex-wrap items-center gap-2">
           <div className="mr-auto flex min-w-0 items-baseline gap-2"><h2 className="text-base font-bold">Jobs</h2><p className="truncate text-[11px] font-medium text-muted-foreground">{completedJobs}/{props.jobs.length} concluídos · {props.taskTotals.done}/{props.taskTotals.total} tarefas</p></div>
            <Select value={groupBy} onValueChange={(value) => setGroupBy(value as GroupBy)}><SelectTrigger className="h-9 w-[174px] rounded-md bg-background text-xs font-semibold" aria-label="Agrupar jobs"><ArrowUpDown className="mr-1 h-3.5 w-3.5" /><span className="mr-1 text-muted-foreground">Agrupar:</span><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">Nenhum</SelectItem><SelectItem value="status">Status</SelectItem><SelectItem value="assignee">Responsável</SelectItem><SelectItem value="due">Prazo</SelectItem></SelectContent></Select>
            <div className="flex h-9 rounded-md bg-muted p-0.5"><Button size="sm" variant={view === "list" ? "secondary" : "ghost"} className="h-8 gap-1.5 px-2.5 text-xs" onClick={() => setView("list")} aria-label="Lista"><List className="h-3.5 w-3.5" />Lista</Button><Button size="sm" variant={view === "board" ? "secondary" : "ghost"} className="h-8 gap-1.5 px-2.5 text-xs" onClick={() => setView("board")} aria-label="Quadro"><Kanban className="h-3.5 w-3.5" />Quadro</Button></div>
            {searchOpen || props.search ? <div className="relative w-[210px]"><Search className="pointer-events-none absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" /><Input autoFocus value={props.search} onChange={(event) => props.onSearchChange(event.target.value)} onBlur={() => { if (!props.search) setSearchOpen(false); }} placeholder="Buscar nome ou número" className="h-9 pl-8 text-xs" /></div> : <Button size="icon" variant="outline" className="h-9 w-9" aria-label="Buscar jobs" title="Buscar jobs" onClick={() => setSearchOpen(true)}><Search className="h-3.5 w-3.5" /></Button>}
      ...
      {props.loading ? <div className="space-y-2 p-4"><Skeleton className="h-16" /><Skeleton className="h-16" /><Skeleton className="h-16" /></div> : props.jobs.length === 0 ? <div className="p-12 text-center text-sm text-muted-foreground">Nenhum job encontrado.</div> : view === "board" ? (
        ...
        ) : <div className={cn("px-4 pb-4", groupBy !== "none" && "space-y-3")}>{groups.map((group) => <section key={group.key}>{groupBy !== "none" ? <header className="flex items-center gap-2 px-1 py-2"><StatusDot color={group.color} round /><h3 className="text-[11px] font-bold uppercase text-muted-foreground">{group.label}</h3><Badge variant="secondary" className="h-5 min-w-5 justify-center rounded-full px-1.5 text-[10px]">{group.jobs.length}</Badge>{groupBy === "status" ? <Button size="icon" variant="ghost" className="ml-auto h-6 w-6" title={`Adicionar job em ${group.label}`} aria-label={`Adicionar job em ${group.label}`} onClick={() => setAddingTo(group.statusId)}><Plus className="h-3.5 w-3.5" /></Button> : null}</header> : null}<div className="overflow-hidden rounded-md border border-border/60 bg-background shadow-sm">{group.jobs.map((job) => <JobRow key={job.id} brandId={props.brandId} job={job} stats={props.stats.get(job.id) ?? { total: 0, done: 0, minutes: 0, assignees: [] }} rollup={props.rollups.get(job.id)} status={statusMap.get(job.status_id ?? "")} team={props.team} onOpen={() => props.onOpen(job.id)} onStatusChange={(id) => props.onStatusChange(job, id)} onAssigneeChange={(id) => props.onAssigneeChange(job, id)} onDueChange={(value) => props.onDueChange(job, value)} menu={props.menuFor(job)} />)}{groupBy === "none" ? <Button variant="ghost" className="h-11 w-full justify-start rounded-none border-t border-dashed border-border/70 px-4 text-xs font-semibold text-primary" onClick={() => setAddingTo(null)}><Plus className="mr-2 h-3.5 w-3.5" />Adicionar um job</Button> : null}</div></section>)}{groupBy !== "none" ? <Button variant="outline" className="h-11 w-full justify-start border-dashed bg-background px-4 text-xs text-muted-foreground" onClick={() => setAddingTo(null)}><Plus className="mr-2 h-3.5 w-3.5" />Adicionar um job</Button> : null}</div>}
    </div>
  );
}