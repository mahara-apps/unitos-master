...
function ProgressCell({ stats, color }: { stats: JobListStats; color?: string | null }) {
  ...
  return (
    <div className="min-w-[106px]">
      <div className="mb-1 flex items-center gap-1.5 text-[11px] tabular-nums text-muted-foreground"><CheckSquare2 className="h-3.5 w-3.5" />{stats.done}/{stats.total}</div>
...
function JobRow({ brandId, job, stats, rollup, status, team, onOpen, onStatusChange, onAssigneeChange, onDueChange, menu }: {
  ...
  return (
    <div className="grid min-h-14 grid-cols-[auto_minmax(180px,1fr)_auto] items-center gap-2 border-b border-border/50 px-3 py-2 last:border-b-0 lg:grid-cols-[auto_minmax(190px,1fr)_118px_80px_95px_90px_165px_auto]">
      <Badge variant="outline" className="h-6 rounded-md font-mono text-[10px] tabular-nums">#{job.job_number}.1</Badge>
      ...
      <div className="col-span-3 flex items-center justify-between gap-3 lg:hidden"><ProgressCell stats={stats} color={status?.color} /><div className={cn("flex items-center gap-1 text-xs", rollup?.running ? "text-work-done" : "text-muted-foreground")}><Clock3 className="h-3.5 w-3.5" />{formatMinutes(minutes)}</div><JobAvatars job={job} stats={stats} team={team} /></div>
    </div>
  );
}
...
function JobBoardCard({ job, stats, rollup, status, team, menu, onOpen }: { job: ProjectJob; stats: JobListStats; rollup?: JobTimeRollup; status?: WorkStatus; team: TeamOption[]; menu: ReactNode; onOpen: () => void }) {
  ...
  return (
    <article ref={setNodeRef} style={style} {...attributes} {...listeners} className={cn("cursor-grab rounded-md border border-border/60 bg-background p-3 shadow-sm", isDragging && "opacity-40")}>
      <div className="flex items-start gap-2"><Badge variant="outline" className="mt-0.5 rounded-md font-mono text-[9px]">#{job.job_number}.1</Badge><Button variant="ghost" className="h-auto min-w-0 flex-1 justify-start p-0 text-left" onClick={onOpen}><span className="line-clamp-2 text-sm font-semibold">{job.name}</span></Button>{menu}</div>
...
function BoardColumn({ status, jobs, renderCard }: { status: WorkStatus | null; jobs: ProjectJob[]; renderCard: (job: ProjectJob) => ReactNode }) {
  ...
  return (
    <section className="flex w-[278px] shrink-0 flex-col overflow-hidden rounded-md border border-border/60 bg-muted/25">
      <header className="flex items-center gap-2 border-b border-border/60 px-3 py-3"><StatusDot color={status?.color ?? null} /><h3 className="truncate text-xs font-semibold">{status?.name ?? "Sem status"}</h3><Badge variant="outline" className="ml-auto h-5 px-1.5 text-[10px]">{jobs.length}</Badge></header>
    ...
    <div>
      <div className="border-b border-border/60 px-4 py-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Select value={groupBy} onValueChange={(value) => setGroupBy(value as GroupBy)}><SelectTrigger className="h-9 w-[155px] text-xs" aria-label="Agrupar jobs"><ArrowUpDown className="mr-1 h-3.5 w-3.5" /><SelectValue /></SelectTrigger><SelectContent><SelectItem value="status">Status</SelectItem><SelectItem value="assignee">Responsável</SelectItem><SelectItem value="due">Prazo</SelectItem></SelectContent></Select>
          ...
          <div className="flex h-9 rounded-md border border-border bg-muted/30 p-0.5"><TooltipProvider><Tooltip><TooltipTrigger asChild><Button size="icon" variant={view === "list" ? "secondary" : "ghost"} className="h-7 w-8" onClick={() => setView("list")} aria-label="Lista"><List className="h-3.5 w-3.5" /></Button></TooltipTrigger><TooltipContent>Lista</TooltipContent></Tooltip><Tooltip><TooltipTrigger asChild><Button size="icon" variant={view === "board" ? "secondary" : "ghost"} className="h-7 w-8" onClick={() => setView("board")} aria-label="Quadro"><Kanban className="h-3.5 w-3.5" /></Button></TooltipTrigger><TooltipContent>Quadro</TooltipContent></Tooltip></TooltipProvider></div>
      ...
      ) : <div>{groups.map((group) => <section key={group.key} className="border-b border-border/60 last:border-b-0"><header className="flex items-center gap-2 bg-muted/25 px-4 py-2"><StatusDot color={group.color} /><h3 className="text-xs font-semibold">{group.label}</h3><Badge variant="outline" className="h-5 px-1.5 text-[10px]">{group.jobs.length}</Badge>{groupBy === "status" ? <Button size="icon" variant="ghost" className="ml-auto h-6 w-6" aria-label={`Adicionar job em ${group.label}`} onClick={() => setAddingTo(group.statusId)}><Plus className="h-3.5 w-3.5" /></Button> : null}</header>{group.jobs.map((job) => <JobRow key={job.id} brandId={props.brandId} job={job} stats={props.stats.get(job.id) ?? { total: 0, done: 0, minutes: 0, assignees: [] }} rollup={props.rollups.get(job.id)} status={statusMap.get(job.status_id ?? "")} team={props.team} onOpen={() => props.onOpen(job.id)} onStatusChange={(id) => props.onStatusChange(job, id)} onAssigneeChange={(id) => props.onAssigneeChange(job, id)} onDueChange={(value) => props.onDueChange(job, value)} menu={props.menuFor(job)} />)}{group.jobs.length === 0 ? <div className="px-4 py-5 text-xs text-muted-foreground">Nenhum job neste grupo.</div> : null}</section>)}<Button variant="ghost" className="h-11 w-full justify-start rounded-none px-4 text-xs text-muted-foreground" onClick={() => setAddingTo(null)}><Plus className="mr-2 h-3.5 w-3.5" />Adicionar um job</Button></div>}
    </div>
  );
}