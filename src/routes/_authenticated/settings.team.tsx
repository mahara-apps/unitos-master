...
export const Route = createFileRoute("/_authenticated/settings/team")({
	head: () => ({
		meta: [
			...
			{
				property: "og:title",
				content: "Equipe e acesso | Configurações | Unitos",
			},
			{
...
function TeamSettingsPage() {
	...
	const activePortals = (teamQ.data?.portalTokens ?? []).filter(
		(t) => !t.revoked_at,
	).length;
		...
		<div className="w-full space-y-4 px-4 py-6 sm:px-6 lg:px-8">
			...
			<Card>
				<CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
					<div>
						...
						<CardDescription>
							Pessoas com acesso a esta marca. O papel define o acesso real
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Convites ainda não aceitos ou expirados.
					</CardDescription>
				</CardHeader>
				<CardContent className="p-0">
					...
					{revokedInvites.length > 0 && (
						<div className="border-t border-border/60 px-6 py-2.5">
							...
							>
								{showRevoked
									? "Ocultar revogados"
									: `Ver revogados (${revokedInvites.length})`}
							</button>
							{showRevoked && (
...
function MemberRow({
	...
	const toggleMut = useMutation({
		mutationFn: () =>
			save({
				data: { brandId, userId: member.userId, isActive: !member.isActive },
			}),
		onSuccess: () => {
			toast.success(
				member.isActive ? "Membro desativado." : "Membro reativado.",
			);
			invalidate();
		},
		onError: (e: Error) =>
			...
			<div className="flex min-w-0 items-center gap-3">
				...
				<div className="min-w-0">
					...
					<div className="mt-0.5 text-[11px] text-muted-foreground">
						Desde {fmtDate(member.createdAt)} · último acesso{" "}
						{fmtDateTime(member.lastSignInAt)}
					</div>
					...
					<AlertDialogContent>
						<AlertDialogHeader>
							...
							<AlertDialogDescription>
								...
								perde o acesso a esta marca e a todos os clientes dela,
								incluindo os vínculos por cliente. A conta de login continua
								existindo, mas sem acesso aqui. Para suspender temporariamente,
								use “Desativar acesso”.
							</AlertDialogDescription>
						</AlertDialogHeader>
						<AlertDialogFooter>
...
function InviteRow({
	...
	const revokeMut = useMutation({
		mutationFn: () =>
			revoke({
				data: { brandId, inviteId: invite.id, confirmLabel: invite.email },
			}),
		onSuccess: () => {
			setRevokeOpen(false);
	...
	const resendMut = useMutation({
		...
		onError: (e: Error) =>
			toast.error("Não foi possível reenviar", { description: e.message }),
	});
