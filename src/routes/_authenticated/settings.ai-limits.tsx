...
function StatusBadge({ spent, limit, hardStop, notifyAtPct }: {
  spent: number; limit: number | null | undefined;
  hardStop: boolean | null | undefined; notifyAtPct: number | null | undefined;
}) {
  const p = pct(spent, limit);
  if (p == null) return <Badge variant="outline" className="text-[10px]">Sem limite</Badge>;
  if (p >= 100 && hardStop) return <Badge variant="destructive" className="text-[10px]"><ShieldAlert className="mr-1 h-3 w-3" />Bloqueado</Badge>;
  if (p >= (notifyAtPct ?? 80)) return <Badge className="bg-amber-500 text-[10px] hover:bg-amber-500">Atenção</Badge>;
...
function AiLimitsPage() {
  ...
  const openBrand = () => setDialog({
    open: true, scope: "brand", targetLabel: "Agência",
  ...
  return (
    <DashboardPageShell>
          ...
          return (
            <div key={c.client_id}>
              <ScopeRow
                ...
                expanded={expanded}
                onToggle={() => setExpandedClients((s) => ({ ...s, [c.client_id]: !s[c.client_id] }))}
                onEdit={() => setDialog({
                  open: true, scope: "client", clientId: c.client_id, targetLabel: c.client_name,
                  limitUsd: c.limit != null ? Number(c.limit) : (overview.brand.limit != null ? Number(overview.brand.limit) : 0),
              ...
              {expanded &&
                users.map((u) => (
                  <ScopeRow
                    ...
                    onEdit={() => setDialog({
                      open: true, scope: "user",
                      clientId: c.client_id,
                      userId: u.user_id,
                      targetLabel: `${u.display_name || u.email || "Usuário"} · ${c.client_name}`,
                      limitUsd: u.limit != null ? Number(u.limit) : (c.limit != null ? Number(c.limit) : 0),
      ...
      <p className="text-xs text-muted-foreground">
        Período: mês atual. Ao atingir 100% de um limite com bloqueio ativado, novas
        execuções de IA no escopo bloqueado passam a falhar até que o limite seja aumentado ou
        o novo mês reinicie a contagem.
      </p>
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="space-y-2">
            ...
            <Slider value={[notify]} min={10} max={100} step={5}
        ...
        <DialogFooter>
          ...
          <Button
            onClick={() => onSave({ limitUsd: Number(limit) || 0, hardStop: hard, notifyAtPct: notify })}
            disabled={saving}
          >
