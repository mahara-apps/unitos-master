...
export const Route = createFileRoute("/_authenticated/pipelines")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Fluxo visual do cliente com colunas customizáveis e D&D." },
    ],
  }),
  ...
  if (!brandId) {
    return (
      <EmptyState
        ...
        description="Escolha um workspace na barra lateral para visualizar a jornada."
      />
    );
  }
  if (!clientId) {
    return (
      <EmptyState
        ...
        description="A jornada é organizada por cliente. Selecione uma conta ativa."
      />
    );
  }

  return <JourneyReady brandId={brandId} clientId={clientId} />;
}

function JourneyReady({ brandId, clientId }: { brandId: string; clientId: string }) {
  ...
  usePageHeader(
    {
      title: "Jornada",
      subtitle: "Fluxo visual do cliente. Colunas customizáveis com D&D fluido.",
      actions: (
